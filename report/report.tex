...
Bus Type: PQ: 24, PV: 5, REF: 1
    ...
    对于一个$n=N-1$节点的系统来说，设其中共有$r$个PV节点，则共有$n-r$个PQ节点。

    在直角坐标的情况下，待求变量共$2n$个，用
    $$x=[\vect{e}^T \vect{f}]^T=[e_1,e_2,...,e_n,f_1,f_2,...,f_n]^T$$ 表示，潮流方程为
    \begin{equation}
      \begin{cases}
        \Delta P_i = P^{SP}_i-(e_a a_i+ f_i b_i) = 0 \; i=1,2,...,n\\
        \Delta Q_i = Q^{SP}_i - (f_i a_i - e_i b_i) = 0 \; i = 1,2,...,n-r\\
      \end{cases}
    \end{equation}
